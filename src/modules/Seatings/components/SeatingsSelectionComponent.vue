<template>
  ...
  <div class="row">
    ...
    <div class="row">
    <div
      :class="['seatContainer', 'col-md-3']" 
      v-for="(seat, index) in seatings" 
      :key="index" 
      @click="onSelectedSeat(seat)">
      <h5 :class="[(index + 1) < 10 ? 'oneDigitMargin': 'twoDigistMargin', getImageClass(seat, true)]">{{ index + 1 }}</h5>
...
import { getOccupiedSeats } from '../actions';
export default {
  ...
  setup (props) {
    ...
    const getSeatings = () => {
      ...
      .then(response => {
        ...
        for(let i = 1; i <= props.travel.bus.capacity; i++) {
          ...
          seatings.value.push({isOccupied: !!seat, isSelected: false, seatNumber: i, seatClass: 'seatFree'});
        }
      })
    ...
    return {
      ...
      onNext: () => {
        ...
        if (selectedSeats.length === 0) alert("Selecciona al menos un asiento para continuar");
        else {
          props.onNextStep(selectedSeats);
        }     
      },
      onSelectedSeat: (seat) => {
        if (seat.isOccupied) alert("El asiento que seleccionaste está ocupado.");
        else seat.isSelected = !seat.isSelected;
      },
      getImageClass: (seat, isNumber=false) => {
        let className = "";
        if (seat.isSelected) className = isNumber ? "selectSeatNumber" : "seatSelect";
        else if (seat.isOccupied) className = isNumber ? "occupiedSeatNumber" : "seatOcuppied";
